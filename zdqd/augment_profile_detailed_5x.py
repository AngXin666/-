...
def augment_image_yolo(image_path, label_path, output_img_dir, output_label_dir, base_name):
    ...
    bright_img_path = output_img_dir / f"{base_name}_bright1.png"
    bright_label_path = output_label_dir / f"{base_name}_bright1.txt"
    ...
    bright_img_path = output_img_dir / f"{base_name}_bright2.png"
    bright_label_path = output_label_dir / f"{base_name}_bright2.txt"
    bright_img.save(bright_img_path)
    with open(bright_label_path, 'w', encoding='utf-8') as f:
        f.write(labels)
    augmented_count += 1
    ...
    contrast_img_path = output_img_dir / f"{base_name}_contrast.png"
    contrast_label_path = output_label_dir / f"{base_name}_contrast.txt"
    ...
    # 5. 组合增强：亮度+对比度
    enhancer = ImageEnhance.Brightness(img)
    combo_img = enhancer.enhance(0.9)
    enhancer = ImageEnhance.Contrast(combo_img)
    combo_img = enhancer.enhance(1.1)
    combo_img_path = output_img_dir / f"{base_name}_combo.png"
    combo_label_path = output_label_dir / f"{base_name}_combo.txt"
...
def main():
    ...
    print(f"  增强倍数: 5倍")
    print(f"  预计生成: {len(original_images) * 5} 张")
