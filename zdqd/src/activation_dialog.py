"""
激活对话框（使用完整版许可证管理器，支持在线验证）
Activation Dialog with Online Verification
"""
...
class ActivationDialog:
    ...
    def _activate(self):
        ...
        try:
            ...
            success, message = self.license_manager.activate_license(license_key, machine_id)
            ...
            self.dialog.config(cursor="")
            messagebox.showerror("激活错误", f"激活过程出错: {e}", parent=self.dialog)
            # 重新启用按钮
            for widget in self.dialog.winfo_children():
                if isinstance(widget, tk.Frame):
                    for child in widget.winfo_children():
