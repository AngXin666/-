...
class AutomationGUI:
    ...
    def _auto_load_accounts(self):
        ...
        try:
            ...
            # [2026-02-22] 修复：清空表格前先保存勾选状态（按手机号保存）
            old_selections = {}
            for item_id in self.results_tree.get_children():
                values = self.results_tree.item(item_id, 'values')
                if values and len(values) > 0:
            ...
            # ===== 步骤8: 恢复勾选状态 =====
            # [2026-02-22] 修复：优先使用刚才保存的勾选状态，如果没有则从配置文件加载
            saved_selections = self.selection_manager.load_selections()
            # 合并两个来源的勾选状态：old_selections（刚才保存的）优先级更高
            if old_selections:
                for phone, checked in old_selections.items():
                    saved_selections[phone] = checked
