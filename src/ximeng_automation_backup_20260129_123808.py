...
class XimengAutomation:
    ...
    def __init__(self, ui_automation: UIAutomation, screen_capture: ScreenCapture, 
        ...
        self.hybrid_detector = PageDetectorHybridOptimized(self.adb, log_callback=log_callback)
        self.navigator = Navigator(self.adb, self.hybrid_detector)
        self.balance_reader = BalanceReader(self.adb)
        self.daily_checkin = DailyCheckin(self.adb, self.hybrid_detector, self.navigator)
        
        # 初始化ProfileReader，传入YOLO检测器
        yolo_detector = self.hybrid_detector._yolo_detector if hasattr(self.hybrid_detector, '_yolo_detector') else None
        if yolo_detector:
            print(f"[初始化] ✓ YOLO检测器已传递给ProfileReader")
        else:
            print(f"[初始化] ✗ YOLO检测器未初始化或传递失败")
        self.profile_reader = ProfileReader(self.adb, yolo_detector=yolo_detector)
        
        # 初始化页面状态守卫
        self.guard = PageStateGuard(self.adb, self.hybrid_detector)
        
        # 初始化OCR增强器
        from .ocr_enhancer import get_ocr_enhancer
        self._ocr_enhancer = get_ocr_enhancer()
        
        # 初始化转账模块
        from .balance_transfer import BalanceTransfer
        self.balance_transfer = BalanceTransfer(self.adb, self.hybrid_detector)
    ...
    async def get_balance(self, device_id: str, from_cache_login: bool = False) -> Optional[float]:
        ...
        try:
            ...
            # 如果不在个人页面，点击底部导航栏的"我的"按钮
            if from_cache_login:
                self._silent_log.info(f" 点击底部导航栏'我的'按钮...")
                # 底部导航栏"我的"按钮坐标 (540x960)
                MY_TAB = (450, 920)
            ...
            # 先检测当前页面状态（诊断用）- 使用优先级模板（只检测个人页相关）
            self._silent_log.info(f" 检测当前页面状态...")
            balance_templates = ['已登陆个人页.png', '未登陆个人页.png', '首页.png']
            page_result = await self.hybrid_detector.detect_page_with_priority(
                device_id, balance_templates, use_cache=True
            )
            self._silent_log.info(f" 当前页面: {page_result.state.value} (置信度: {page_result.confidence:.2f})")
            
            # 使用 profile_reader 获取余额（更可靠）
        ...
        try:
            ...
            if result['success']:
                ...
                sign_in_result = SignInResult(
                    success=True,
                    already_signed=result.get('already_checked', False)
                )
                # 添加额外信息
            ...
            elif result.get('already_checked', False):
                log("✓ 今日已签到")
                sign_in_result = SignInResult(
                    success=True,
                    already_signed=True
            ...
            log(f"✗ 签到异常: {e}")
            return SignInResult(success=False, error_message=str(e))
    ...
    async def _check_and_auto_transfer(self, device_id: str, phone: str, log_callback=None):
        ...
        try:
            ...
            success = await self.guard.ensure_page_state(
                device_id,
                PageState.PROFILE_LOGGED,
                self.navigator.navigate_to_personal,
                "自动转账前进入个人页"
            )
            ...
            log(f"  [自动转账] ✓ 满足转账条件，准备转账到 ID: {recipient_id}")
    ...
    async def run_full_workflow(self, device_id: str, account: Account, skip_login: bool = False) -> AccountResult:
        ...
        try:
            ...
            if skip_login:
                ...
                points_check_templates = ['积分页.png', '首页.png', '已登陆个人页.png', '模拟器桌面.png']
                page_result = await self.hybrid_detector.detect_page_with_priority(
                    device_id, points_check_templates, use_cache=False
                )
                ...
                if page_result and page_result.state == PageState.POINTS_PAGE:
                    ...
                    for retry in range(3):
                        # 使用完整检测（模板+OCR），更准确判断页面状态
                        page_result = await self.hybrid_detector.detect_page(
                            device_id, use_ocr=True, use_template=True
                        )
            ...
            for attempt in range(3):
                try:
                    ...
                    if skip_login:
                        ...
                        # 立即检测页面状态（不等待）
                        from .page_detector import PageState
                        page_result = await self.hybrid_detector.detect_page(device_id, use_dl=True)
                        ...
                        if page_result.state != PageState.PROFILE_LOGGED:
                            ...
                            # 使用全局便捷函数等待页面完全加载
                            # 检测页面稳定性：连续2次检测到PROFILE_LOGGED状态
                            log(f"  等待页面完全加载...")
                    ...
                    if not nav_success:
                        ...
                        # 检查当前页面状态 - 使用优先级模板（个人页相关）
                        from .page_detector import PageState
                        profile_check_templates = ['已登陆个人页.png', '未登陆个人页.png', '首页.png']
                        page_result = await self.hybrid_detector.detect_page_with_priority(
                            device_id, profile_check_templates, use_cache=True
                        )
                        ...
                        if page_result and page_result.state:
                            log(f"  页面确认: {page_result.state.value}")
                            if page_result.state != PageState.PROFILE_LOGGED:
                                log(f"  ⚠️ 页面状态异常，当前不在个人页（已登录）")
                                if attempt < 2:
                                    await asyncio.sleep(2)
                                    continue
                        else:
            ...
            if self.auto_login.enable_cache and self.auto_login.cache_manager:
                ...
                try:
                    cache_saved = await self.auto_login.cache_manager.save_login_cache(
                        device_id, 
                        account.phone, 
                        user_id=result.user_id  # 即使是 None 也传递
                    )
                    if cache_saved:
                        log(f"✓ 登录缓存已保存")
                    else:
            ...
            try:
                ...
                checkin_result = await self.daily_checkin.do_checkin(
                    ...
                    log_callback=None,
                    profile_data=profile_data  # 传递已获取的个人信息
                )
                ...
                if checkin_result['success']:
                    ...
                    log(f"⚠️ 签到失败: {checkin_result.get('message', '未知错误')}\n")
            ...
            try:
                # 导航回个人资料页面（带超时）
                nav_task = self.navigator.navigate_to_profile(device_id)
                try:
                    nav_success = await asyncio.wait_for(nav_task, timeout=10.0)
                ...
                if not nav_success:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 更新最终数据
                            result.balance_after = final_profile.get('balance')
                            # 更新其他可能变化的字段
