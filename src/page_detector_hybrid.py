...
class PageDetectorHybrid:
    ...
    def __init__(self, adb: ADBBridge, log_callback=None):
        ...
        try:
            ...
            self._dl_detector = PageDetectorDL(adb, log_callback=log_callback)
            if self._dl_detector._model:
                # 静默初始化成功
                pass
            else:
                # 静默初始化失败
        ...
        # 初始化YOLO检测器
        try:
            from .yolo_button_detector import YoloDetector
            self._yolo_detector = YoloDetector(adb, log_callback=log_callback)
            print(f"[PageDetectorHybrid] ✓ YOLO检测器初始化成功")
        except Exception as e:
            print(f"[PageDetectorHybrid] ✗ YOLO检测器初始化失败: {e}")
    ...
    async def _detect_by_template(self, device_id: str) -> Optional[PageDetectionResult]:
        ...
        try:
            ...
            # 提高相似度阈值到90%，避免误判（原来80%太低，导致把加载页误判为登录页）
            if best_match and best_match['similarity'] >= 0.90:
                ...
                if best_match:
                    ...
                    if best_match['similarity'] >= 0.85:
                        print(f"    [模板匹配] 相似度过低: {best_match['template_name']} ({best_match['similarity']:.2%} < 90%)，降级到像素检测")
    ...
    async def detect_page(self, device_id: str, use_ocr: bool = False, use_template: bool = True, use_dl: bool = True) -> PageDetectionResult:
        """检测当前页面状态（优化版：缓存 -> 深度学习 -> 模板匹配 -> 像素检测 -> OCR）
        
        Args:
            device_id: 设备 ID
            use_ocr: 是否使用 OCR（默认只用像素检测）
            use_template: 是否使用模板匹配（默认True）
            use_dl: 是否使用深度学习检测（默认True，优先级最高）
        ...
        if cached_result is not None:
            # 缓存命中，直接返回（不输出日志，减少干扰）
            cached_result.cached = True
            cached_result.detection_time = time.time() - start_time
            return cached_result
        
        # 策略1: 深度学习检测（最准确，速度快，优先级最高）
        if use_dl and hasattr(self, '_dl_detector') and self._dl_detector:
            dl_result = await self._dl_detector.detect_page(device_id, use_cache=False)
            # 深度学习置信度阈值：0.85（降低阈值，提高深度学习优先级）
            # 训练时准确率100%，实际使用时置信度通常>95%
            if dl_result and dl_result.confidence >= 0.85:
                # 深度学习检测成功且置信度高，直接返回
                dl_result.detection_time = time.time() - start_time
                dl_result.cached = False
                # 静默检测，不输出日志
                
                # 更新缓存
                self._detection_cache.set(device_id, dl_result)
                return dl_result
        
        # 策略2: 模板匹配（快速，准确率高，但不够灵活）
        if use_template and self._template_matcher:
            template_result = await self._detect_by_template(device_id)
            # 提高置信度阈值到0.90，避免误判（原来0.80太低，导致把加载页误判为登录页）
            if template_result and template_result.confidence >= 0.90:
                # 模板匹配成功且置信度高，直接返回
    ...
    async def detect_page_with_priority(self, device_id: str, 
        ...
        # 策略1: 深度学习检测（最准确，优先级最高）
        if hasattr(self, '_dl_detector') and self._dl_detector:
            dl_result = await self._dl_detector.detect_page(device_id, use_cache=False)
            # 深度学习置信度阈值：0.85
            if dl_result and dl_result.confidence >= 0.85:
                dl_result.detection_time = time.time() - start_time
    ...
    async def _close_popup_impl(self, device_id: str) -> bool:
        ...
        if popup_type == "home_announcement":
            ...
            # 检查是否成功关闭
            result = await self.detect_page(device_id, use_ocr=False)  # 不使用OCR
            if result.state != PageState.POPUP:
                ...
                await asyncio.sleep(1.0)  # 减少等待时间
                
                result = await self.detect_page(device_id, use_ocr=False)
                if result.state != PageState.POPUP:
                    print(f"    [弹窗] ✓ 成功关闭首页公告弹窗")
                    return True
        ...
        if is_checkin_popup:
            ...
            # 优化：只使用第一个最准确的位置，如果失败再尝试其他位置
            primary_pos = self.CHECKIN_POPUP_CLOSE[0]
            print(f"    [弹窗] 尝试主要位置: {primary_pos}")
            await self.adb.tap(device_id, primary_pos[0], primary_pos[1])
            await asyncio.sleep(0.5)  # 减少等待时间：0.8秒 -> 0.5秒
            
            # 快速检查是否成功关闭（只用模板匹配，不用OCR）
            result = await self.detect_page(device_id, use_ocr=False, use_template=True, use_dl=False)
            if result.state != PageState.POPUP:
                print(f"    [弹窗] ✓ 成功关闭签到弹窗（主要位置）")
                return True
            
            # 如果主要位置失败，尝试备用位置
            print(f"    [弹窗] 主要位置失败，尝试备用位置...")
            for i, (x, y) in enumerate(self.CHECKIN_POPUP_CLOSE[1:], 2):
        ...
        if button_pos:
            ...
            if popup_type in ["unknown", "home_announcement", "user_agreement"]:
                ...
                for pos in alternative_positions:
                    ...
                    await asyncio.sleep(0.8)  # 减少等待时间：1.5秒 -> 0.8秒
    ...
    async def detect_buttons_yolo(self, device_id: str, page_type: str, 
        ...
        return await self._yolo_detector.detect(device_id, page_type, conf_threshold)
    ...
    async def find_button_yolo(self, device_id: str, page_type: str, button_name: str,
        ...
        return await self._yolo_detector.find_object(device_id, page_type, button_name, conf_threshold)
    
    async def click_button_yolo(self, device_id: str, page_type: str, button_name: str,
        ...
        return await self._yolo_detector.click_object(device_id, page_type, button_name, conf_threshold)
