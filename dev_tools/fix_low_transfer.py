...
使用公式：转账金额 = 前天余额 + 当时签到奖励 - 最终余额

运行方式:
    python dev_tools/fix_low_transfer.py
"""
...
def fix_low_transfer():
        ...
        for idx, record in enumerate(records):
            ...
            if 0 < transfer_amount < MIN_TRANSFER_AMOUNT:
                ...
                if balance_after is not None:
                    ...
                    # 如果差异超过0.01元，需要修复
                    if abs(correct_transfer - transfer_amount) > 0.01:
                        need_fix.append({
                            'id': record.get('id'),
                            'phone': phone,
    ...
    for item in need_fix:
        print(f"修复账号 {item['phone']}, 日期 {item['date']}")
        print(f"  前天余额: {item['prev_balance']:.2f}")
        print(f"  签到奖励: {item['old_checkin_reward']:.2f}")
        print(f"  最终余额: {item['balance_after']:.2f}")
        print(f"  当前转账: {item['current_transfer']:.2f}")
        print(f"  正确转账: {item['correct_transfer']:.2f}")
