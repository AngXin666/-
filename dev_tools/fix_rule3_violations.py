...
运行方式:
    python dev_tools/fix_rule3_violations.py
"""
...
def fix_rule3_violations():
    ...
    for phone, records in records_by_phone.items():
        for record in records:
            ...
            if checkin_balance_after is not None and balance_after is not None:
                expected = checkin_balance_after - transfer_amount
    ...
    print(f"发现 {len(violations)} 条违反规则3的记录")
    print()
    ...
    for v in violations:
        ...
        print(f"  balance_after: {v['balance_after']:.2f} → {v['expected']:.2f}")
