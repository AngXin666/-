...
通用页面分类器训练脚本
训练所有33个类别的页面分类器
使用MobileNetV3架构，支持GPU加速、混合精度训练
"""
import os
import sys
...
class GeneralPageDataset(Dataset):
    """通用页面分类器数据集 - 加载所有类别"""
...
def main():
    ...
    print(f"  • 混合精度训练(AMP): {'已启用 ⚡' if device.type == 'cuda' else '未启用'}")
    ...
    # 加载数据集
    print(f"\n📦 加载数据集...")
    dataset = GeneralPageDataset(training_data_dir, transform=transform)
