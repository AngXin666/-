...
# [2026-03-01] 移除固定随机种子，避免因随机性不足导致页面识别错误
# 让训练过程保持随机性，提高模型泛化能力
...
def main():
    """主训练函数"""
    print("\n" + "=" * 80)
    print("🎯 页面分类器选择性训练 (签到流程页面)")
    print("=" * 80)
    
    # [2026-03-01] 不固定随机种子，保持训练随机性，提高模型泛化能力
    print(f"\n🎲 随机种子: 未固定 (保持随机性)")
    
    # 配置
    script_dir = Path(__file__).parent
    training_data_dir = Path("标注工具_完整独立版/training_data")
    models_dir = Path("标注工具_完整独立版/models")
    ...
    if device.type == 'cuda':
        ...
        # [2026-03-01] 启用benchmark模式以提高训练速度
    ...
    # [2026-03-01] 不固定生成器种子，保持数据划分的随机性
    ...
    best_val_acc = 0.0
    best_model_path = models_dir / "page_classifier_checkin_best.pth"
    ...
    # 保存类别列表
    classes_path = models_dir / "page_classes_checkin.json"
