...
def main():
    base_dir = Path(__file__).parent.parent / "标注工具_完整独立版" / "training_data"
    ...
    for category in CATEGORIES:
        ...
        print(f"  • 增强后总数: {original_count + augment_count} 张")
    ...
    for category in CATEGORIES:
        ...
        if category_dir.exists():
            all_images = list(category_dir.glob("*.png"))
            print(f"  • {category}: {len(all_images)} 张")
